...
public class KeyListAdapter extends RecyclerView.Adapter<KeyListAdapter.CustomViewHolder>{
    ...
    private IKeyListView iKeyListView;

    public KeyListAdapter(IKeyListView iKeyListView, ArrayList<IKey> items) {
        super();
        this.items = items;
        this.iKeyListView = iKeyListView;
    }

    @NonNull
    @Override
    public KeyListAdapter.CustomViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new KeyListAdapter.CustomViewHolder(LayoutInflater.from(iKeyListView.getContext()).inflate(R.layout.row_key, parent, false));
    }
    ...
    public void onBindViewHolder(@NonNull KeyListAdapter.CustomViewHolder holder, int position) {
        holder.fioTV.setText(items.get(position).getFio());
        holder.keyTV.setText(items.get(position).getKeyBytes());
        holder.view.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                iKeyListView.startEditKey(position);
            }
        });
        holder.view.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                iKeyListView.startControlKeyDialogFragment(position);
                return true;
            }
        });
    }
